...
## Follow-up: Bug navigasi pasca-migrasi — mobile-web back → blank screen (ditemukan 2026-07-09)

Setelah migrasi di atas selesai, user melaporkan: setelah "Get Started" di landing page lalu pilih menu sidebar, tekan **back di mobile web** berakhir di **blank screen**, bukan kembali ke menu sidebar. Ini persis risiko yang sudah diberi tanda di `handoff.md` assumption #5 ("kept `navigator.navigateTo(ThreePaneScaffoldRole.Primary, ...)` ... judgment call, not verified") — tapi ternyata arah forward (klik sidebar) sudah benar, yang hilang adalah sinkronisasi ke arah sebaliknya (back).

**Root cause**: `ContentScreen.kt` punya dua sumber state "posisi sekarang" yang cuma sinkron satu arah — `detailBackStack` (isi konten aktual) dan `navigator`/`ThreePaneScaffoldNavigator` (pane mana yang tampil di layar sempit). `navigator.navigateTo(Primary, ...)` cuma dipanggil dari `onSidebarClick` (maju). `ChronologicalBrowserNavigation` (web-only, browser back/forward) memutasi `detailBackStack` langsung tanpa pernah menyentuh `navigator` — jadi begitu `detailBackStack` collapse balik ke placeholder `DefaultNavigation` (kotak kosong), `navigator` tetap nyangkut di detail pane yang sekarang blank.

**Rencana perbaikan** (detail lengkap, termasuk hasil verifikasi API `ThreePaneScaffoldNavigator` yang sebenarnya, ada di `handoff.md` bagian "Status as of 2026-07-09 (post-migration bug found...)" dan di file plan sesi terkait): ganti sinkronisasi imperatif (dipanggil manual di titik klik/back) jadi reaktif — satu `LaunchedEffect` yang selalu menyamakan pane role `navigator` dengan entry teratas `detailBackStack`, apapun penyebab perubahannya (klik sidebar, back handler in-app, atau browser back/forward). Scope perubahan: `ContentScreen.kt` saja, tidak ada perubahan di `webMain/main.kt` atau `Sidebar.kt`.
